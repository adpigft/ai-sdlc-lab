...
Every capability may contain:

```text
domains/<domain>/capabilities/<capability>/workflow-state.yaml
```

Use `framework/workflow/workflow-state-template.yaml` when creating a new workflow-state file.
...
When a lifecycle skill creates or updates its owned artifact, it must create or update `workflow-state.yaml` in the capability folder.
...
| `context/context.md` or ADR/API design artifact | `architecture_review` | `architecture_approval` |
