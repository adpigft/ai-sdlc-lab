...
| Specification Draft | Business Analyst | Product Owner | 4 | Update or preview Jira Story details for specification refinement. | Publish specification summary preview only. | Read spec draft status and required downstream gates. | Trigger if spec draft is stale, missing approval path, or blocked. |
| Specification Approved | Business Analyst | Product Owner | 2 | Keep Jira Story aligned with approved specification and traceability. | Publish approved specification summary. | Read approved spec evidence and downstream readiness. | Trigger if design does not begin within threshold or traceability is incomplete. |
