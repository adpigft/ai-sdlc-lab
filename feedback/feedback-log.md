...
| FB-QRREF-003 | 2026-06-01 | Implementation plan review | QR Refund | Implementation plan needed tighter MVP boundaries, aggregate ownership clarity, durable audit outbox selection, dependency-specific failure behavior, hashed idempotency storage, aggregate versioning, minimum CI gates, API error contract tests, and minimal adapter guidance. | Without correction, developers could overbuild projection integrations or start blocked slices before required ADR/config approvals. | JIRA-QRREF-090 | CONF-PAY-QRREF-IMPLEMENTATION | Updated implementation plan to separate startable, blocked, and future-phase slices; clarified domain ownership, audit outbox, failure handling, idempotency, CI gates, and testing expectations. | Actioned in implementation plan | Developer Lead |
| FB-QRREF-004 | 2026-06-01 | Implementation slice definition review | QR Refund | Implementation plan needed explicit six-slice sequencing and traceability from slice to requirements, APIs/interfaces, tests, and status. | Without slice-level governance, developers could mix blocked processor, ledger, override, reconciliation, or reporting work into the startable refund creation foundation. | JIRA-QRREF-090 | CONF-PAY-QRREF-IMPLEMENTATION | Defined all six implementation slices, added the one-approved-slice-at-a-time rule, added blocked-dependency stop guidance, and updated traceability with slice mappings. | Actioned in implementation plan and traceability | Developer Lead |
